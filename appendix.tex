...
\begin{enumerate}
  ...
  \item \textbf{Run the notebooks.} Recreate the notebooks of Section~\ref{app:mnn-nb}
    and execute them (or run \texttt{REPRODUCE\_ALL}). MNIST downloads automatically; the
    benchmark and store-recall grids are replayed from \texttt{data/results/}.
...
map and the settings of Table~\ref{table:A6}; (ii)~\texttt{temporal\_storerecall} replays the
temporal-benchmark capability and retention grids and the store-recall network, showing the
...
for name in notebooks:
    ...
    client = NotebookClient(nb, timeout=2400, kernel_name="mnn",
